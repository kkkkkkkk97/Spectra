...
class RecordLayer(object):
    ...
    def _macThenEncrypt(self, data, contentType):
        ...
        if self._writeState.macContext:
            # print("self._writeState.macContext:")
        ...
        if self._writeState.encContext:
            ...
            #Encrypt
            # print("self._writeState.encContext.encrypt(data)")
    ...
    def _getNonce(self, state, seqnum):
        ...
        # print("state.fixedNonce",state.fixedNonce,len(state.fixedNonce))
    ...
    def _decryptAndUnseal(self, header, buf):
        ...
        # print(buf)
        decrypt_buf = self._readState.encContext.open(nonce, buf, authData)
        if decrypt_buf is None:
    ...
    def recvRecord(self):
        ...
        while True:
            ...
            # Flag to track if alert was successfully decrypted
            alert_decryption_failed = False
            ...
            try:
                ...
                # message at any point in connection
                    ...
                    pass
                # Allow plaintext alerts even when encryption is active
                elif header.type == ContentType.alert and \
                        self._readState and self._readState.encContext:
                    # Try to decrypt, but if it fails, accept as plaintext
                    try:
                        if self._readState.encContext.isAEAD:
                            data = self._decryptAndUnseal(header, data)
                        elif self._readState.encryptThenMAC:
                            data = self._macThenDecrypt(header.type, data)
                        elif self._readState.encContext.isBlockCipher:
                            data = self._decryptThenMAC(header.type, data)
                        else:
                            data = self._decryptStreamThenMAC(header.type, data)
                    except (TLSBadRecordMAC, TLSDecryptionFailed):
                        # Decryption failed - accept as plaintext alert
                        alert_decryption_failed = True
            ...
            # Skip de-padding for plaintext alerts
            if self._is_tls13_plus() and self._readState and \
                    self._readState.encContext and\
                    header.type != ContentType.change_cipher_spec and \
                    not alert_decryption_failed:
